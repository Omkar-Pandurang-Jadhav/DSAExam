...
void polynomialAddition(struct Node **head1,struct Node **head2,struct Node **head3){
    struct Node *p1=*head1;
    struct Node *p2=*head2;
    struct Node *p3=*head3;
    while(p1!=NULL && p2!=NULL){
        if(p1->pow>p2->pow){
            insertNode(head3,p1->coeff,p1->pow);
            p1=p1->next;
        }
        else if(p1->pow<p2->pow){
            insertNode(head3,p2->coeff,p2->pow);
            p2=p2->next;
        }
        else if(p1->pow==p2->pow){
            insertNode(head3,p2->coeff+p1->coeff,p2->pow);
            p1=p1->next;
            p2=p2->next;
        }
    }
...
int main(){
    ...
    polynomialAddition(&head1,&head2,&head3);

    printf("The linked list 1 is : \n");
    displayLinkedList(head1);
    printf("The linked list 2 is : \n");
    displayLinkedList(head2);
    printf("The polynomial addition of equation is : \n");
